with performance_review_processes as (
    select * from {{ ref("base_factorial_performance_review_processes") }}
),

performance_review_process_targets as (
    select * from {{ ref("base_factorial_performance_review_process_targets") }}
),

performance_reviews_evaluations as (
    select * from {{ ref("base_factorial_performance_reviews_evaluations") }}
),

performance_review_scores as (
    select * from {{ ref("base_factorial_performance_scores") }}
),

final_employee_scores as (
    select * from {{ ref("base_factorial_performance_review_final_employee_scores") }}
),
...
left join employees e
    on cast(prpt.target_access_id as bigint) = cast(e.access_id as bigint)
left join performance_reviews_evaluations self_pre
    on prpt.target_access_id = self_pre.target_access_id 
    and prp.performance_review_process_id = self_pre.review_process_id 
    and self_pre.evaluation_type = 'self'
left join performance_reviews_evaluations manager_pre
    on prpt.target_access_id = manager_pre.target_access_id 
    and prp.performance_review_process_id = manager_pre.review_process_id 
    and manager_pre.evaluation_type = 'manager'
left join performance_review_scores self_score
    on prp.performance_review_process_id = self_score.review_process_id 
    and prpt.target_access_id = self_score.target_access_id 
    and self_score.reviewer_strategy = 'self'
left join performance_review_scores manager_score
    on prp.performance_review_process_id = manager_score.review_process_id 
    and prpt.target_access_id = manager_score.target_access_id 
    and manager_score.reviewer_strategy = 'manager'
...
left join employees manager
    on cast(manager.access_id as bigint) = cast(manager_pre.reviewer_access_id as bigint)
